...
#if defined(NDEBUG)
# error "Qimncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3722bd542b4cbf0cf37287acfb1c8717ec2da563fb9b3e508468c1efbd3e0b16");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Qimncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Qimncoin Signed Message:\n";
...
{
    // Qimncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Qimncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Qimncoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Qimncoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Qimncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Qimncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7e3f2942e59a1f6b64c5cc927016123f79ac6cbbdbe98160c7f8897b33b94d9a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Qimncoin is coming";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0453225f8e3aa847b2fc3d61257417e2753a4de6122c79ba486db63b11ce6919c40debc0ddfaeb9542d7eb48f2f6eab18d64d11ecf3239f75b9214af9f3b3b40c2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519640328;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086681630;

        if (fTestNet)
        {
            block.nTime    = 1519640309;
            block.nNonce   = 386240039;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa65c2d189a6dc7b0afcf34da5d3cfd5b76e16b92801138aca209e9fa280bd9c6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Qimncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QimncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QimncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static QimncoinMiner(CWallet *pwallet)
{
    printf("QimncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("qimncoin-miner");
    ...
    try { loop {
        ...
        printf("Running QimncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("QimncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&QimncoinMiner, pwallet));
}
